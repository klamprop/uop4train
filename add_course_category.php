<?php 
	...
	if(isset($_GET["id"]))
	{
		$query_select = "SELECT name,active FROM tbl_category_courses WHERE id=".$_GET["id"];
...
<div class="row"> <!--  ------------------------  START CONTENT      ------------------------      -->
	...
	<h1>
		...
		<?php if(isset($_GET['id'])) { echo "Edit Course Category"; } else { echo "Add Course Category"; } ?>
	</h1>
	</div>
...
<script>
	...
	var citem="<?php if(isset($_GET['citem'])){echo $_GET['citem'];} ?>";
	...
	/*
		...
		*/
		...
		$.ajax({
			type: "POST",
			url: "functions/create_course_category.php",
			data: data_post,
			dataType: "json",
			success: function(msg){
				if(parseInt(msg.status)==0)
				{
					window.location.href="add_course_category.php"
				}	
				else if(parseInt(msg.status)>0)
				{
					
					window.location.href="course_category.php";
				}
				hideshow('loading',0);					
			}							
		});
