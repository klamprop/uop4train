...
$provider = new \OpenIDConnectClient\OpenIDConnectProvider([
    ...
    'redirectUri'             => 'https://www.securityaware.me/clientsmesec.php',
	...
	'publicKey'               => 'file://keys/smesecpublic.key',
],
    [
        'signer' => $signer
    ]
...
$company        = $idToken->getClaim('company', false);
$allClaims      = $idToken->getClaims();




registerUser($connection, $email, $fname, $lname, $company, "" );
makeUserALoggedInUser($connection, $email);
header('Location: all_course_smesec.php');

function registerUser($connection, $email, $fname, $lname, $company, $picture){
	...
	{
		$passw =  rand();
		$results_register_query = "INSERT INTO tbl_users (name_user, surname_user, email_user, password_user, active_user,register_date,last_login_date, avatar_name, auth_type, company) VALUES ('".
		$fname."', '".$lname."','".$email."',MD5('".$passw."'), 1, now(),now(), '". $picture."','SMESEC','".$company."')";
